...
index1=0
index2=0
array1=[[0] * 2 for i in range(40000)]
array2=[[0] * 4 for i in range(10000)]
...
for i in range(1,1000):
    ...
    check=record[7].strip().replace(" [详细]", "")               #检查项目7
    symptom=record[8].strip().replace(" [详细]","")               #症状8
    complication=record[9].strip().strip('.')                   #并发症9
    period=record[12].strip().strip('.')                        #治疗时间5
    treatment=record[10].strip().replace(" [详细]", "")           #治疗方法10
    drug=record[11].strip().replace(" [详细]","").replace(".","") #用药1
    rate=record[13].strip().replace("暂无相关资料","").replace("暂无相关资料","治愈率")    #治愈率11
